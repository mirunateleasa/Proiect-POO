...
public:
	...
	int searchFile(const char numeTabel[])
	{
		...
		{
			if (strcmp(this->lines[i].c_str(), numeTabel) == 0)
			{
				return i;
				break;
			}
...
private:
	...
	void checkCommand(string commandFirst, string commandSecond)
	{
		if (commandSecond == "TABLE")
		{
			...
			{
				cout << "HELAU YOU ARE HERE AND THE PROBLEM IS THAT THE COMMAND FIRST IS: *" << commandFirst << "* and the command second: *" << commandSecond << "*";
			...
			if (commandSecond == "INDEX")
			{
				...
				if (commandSecond == "")
				{
					...
					{
						cout << "THE COMMAND IS: *" << commandFirst << "* AND *" << commandSecond << "* SO IS NOT CORRECT?";
						throw new InvalidCommandException("No such command!", 0);
					}
				}
				else
...
private:
	...
	void doCreate(string commandName)
	{
		...
		cout <<endl<< "CONGRATS! You have created the following table: " << endl;
		cout << tabelCreat;
		cout << endl << endl;
	}
	void createTheNewDatabase()
...
private:
	void checkDelete(string commandName, string& tableName, string& columnName, string& columnValue)  // "table_name WHERE column_name = value"
	{
		...
		if (function.findChars(columnValue, function.SIGNS) == 1)
		{
			cout << endl << "YOU ARE HERE AND THE COLUMN VALUE IS" << columnValue << endl;
	...
	void doDelete(string commandName)
	{
		...
		cout << endl << "The value you want to delete before checking is: " << this->columnValue;
		...
		else
		{
			...
			int checker = -1;
			//int pozitii = -1;
			//pozitii= this->columnValue.find_first_not_of("0123456789");
			float value = -1;
			// aici daca checker >=0 inseamna ca exista litere deci inseamna ca formatul columnValue era "string" => string, daca nu exista altcv in afara de cifre s-a luat direct ca fiind value 
			// insa poate fi si cazul in care stringul este "10001" caz in care nu exista litere dar tot string si este nevoie de o verificare mai buna dar momentan nu stiu care ar putea fi 
			// cred ca ceva de genul asta
			// cu un getter care ia type-ul ColumnAttribute, de la TableColumn unde name = this->columnName 

			for (int i = 0; i < theTable.getNoColumns(); i++) {
				if (theTable.getColumn(i)->getName() == this->columnName) {
					checker = theTable.getColumn(i)->getType();
				}
			}
			if (checker > 0) {
				this->columnValue = this->columnValue.substr(1, this->columnValue.length() - 2);
			}
			else
			{
				value = stof(this->columnValue);
			}
			...
			for (int i = 0; i < theTable.getNoColumns(); i++) {
				if (theTable.getColumn(i)->getName() == this->columnName)
				{
					for (int j = 0; j < theTable.getColumn(i)->getNoAttributes(); j++) {
						if (value == -1) {
							if (theTable.getColumn(i)->getAttribute(j)->getValue() == this->columnValue) {
								theTable.getColumn(i)->getAttribute(j)->setAttributeString(theTable.getColumn(i)->getDefault());
								p++;
							}
						}
						else {
							if (stof(theTable.getColumn(i)->getAttribute(j)->getValue()) == value) {
								theTable.getColumn(i)->getAttribute(j)->setAttributeFloat((theTable.getColumn(i)->getDefault()));
								p++;
							}
						}
						if (j == (theTable.getColumn(i)->getNoAttributes() - 1) && p == 0)
							throw new InvalidCommandException("Sorry, there is no attribute with this value to delete!", 0);
					}
					c++;
				}
				if (i == (theTable.getNoColumns() - 1) && c == 0)
					throw new InvalidCommandException("Sorry, there is no column with this name to delete from!", 0);
			}
			theTable.writeTableToBin(theTable.getName());
		}
	}
...
private:
	...
	void doInsert(string commandName)
	{
		...
		cout << endl << "CONGRATS! You have inserted the values in the following table: " << theTable.getName();
	}
};
